...
COUNTRY_PRICE_COLUMN = {
    '506': 'Inscripción Costa Rica','598': 'Inscripción Uruguay','595': 'Inscripción Paraguay',
    '591': 'Inscripción Bolivia','54': 'Inscripción Argentina','56': 'Inscripción Chile',
    '57': 'Inscripción Colombia','52': 'Inscripción México','51': 'Inscripción Perú',
}
...
def _best_row_by_query(rows, q_fold: str):
    ...
    for r in rows:
        ...
        if name and q_fold in _fold(name):
            return r
    # 2) o el nombre completo dentro del texto del usuario
    for r in rows:
        name = (r.get('Curso') or '').strip()
        if name and _fold(name) in q_fold:
            return r
    # 3) intersección de tokens (>=3 letras)
...
def find_course(rows, user_text: str):
    ...
    if m:
        ...
        if r:
            print('[BEST MATCH after keyword]', candidate, '->', r.get('Curso'))
            return r
