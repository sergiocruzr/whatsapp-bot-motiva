...
HEADER_SYNONYMS = {'Valor Inscripción Uruguay': 'Inscripción Uruguay'}
...
def fetch_sheet_rows(force=False):
    ...
    # Validacion flexible: todos los requeridos deben estar; Alias puede o no estar
...
# Busqueda de curso (robusta y ASCII)
def _best_row_by_query(rows, q_fold):
    # el texto del usuario dentro del nombre del curso
    for r in rows:
        name = (r.get('Curso') or '').strip()
        if name and q_fold in _fold(name):
            return r
    # o el nombre completo dentro del texto del usuario
    for r in rows:
        name = (r.get('Curso') or '').strip()
...
def find_course(rows, user_text):
    ...
    for a, r in (_cache.get('alias_idx') or {}).items():
        if a and a in q_fold:
            return r
    # patrones tipo "info|precio|horario|pdf <algo>"
...
# Respuestas
def first_response_for_course(row):
    partes = []
...
def answer_faq(row, body_lower):
    ...
    if any(k in body_lower for k in ['modalidad','metodolog','online','virtual','en vivo','zoom','meet']):
        out.append('Modalidad en vivo por videoconferencia (clases sincrónicas).')
...
def whatsapp_webhook():
    try:
        ...
        if row:
            ...
            resp = first_response_for_course(row)
